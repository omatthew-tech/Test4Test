...
export function ReviseSubmissionPage() {
  ...
  const receivedRatingFallback = useMemo(() => {
    ...
    return (
      state.feedbackRatings.find(
        (rating) =>
          rating.testResponseId === response.id && rating.ratedByUserId === submission.userId,
      )?.ratingValue ?? null
    );
  ...
  const [receivedRating, setReceivedRating] = useState<FeedbackRatingValue | null>(
  ...
  const canReport =
    (receivedRating === "frowny" || receivedRating === "neutral") && !isReportInProgress;
    ...
    setAnswers(
      Object.fromEntries(
        response.answers.map((answer) => [
          answer.questionId,
          answer.type === "multiple" ? (answer.selectedOption ?? "") : (answer.textAnswer ?? ""),
        ]),
      ),
    );
  }, [response?.id]);
  ...
  if (!currentUser || !response) {
    return (
      <AppShell title="Revise feedback" eyebrowLabel={null}>
        <Surface>
          <p>That submitted test could not be found.</p>
        </Surface>
      </AppShell>
    );
  }
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      setReportError(
        error instanceof Error ? error.message : "We could not send your report right now.",
      );
    } finally {
    ...
    <AppShell eyebrowLabel={null}>
      <div className="test-layout test-layout--single revise-response-page">
        ...
        <Surface className="test-questions test-questions--full">
          ...
          {message && reportPhase !== "success" ? (
            <div className="callout callout--soft">{message}</div>
          ) : null}
          {isReportInProgress && reportPhase !== "success" ? (
            <div className="callout callout--soft">
              Your report is in progress. This feedback is locked while we review it.
            </div>
          ) : null}
          ...
          {isLoadingQuestions ? (
            <div className="empty-state empty-state--left">
              ...
              <p>
                Pulling in the question set you answered the first time so you can revise it
                cleanly.
              </p>
            </div>
          ) : (
            ...
            {reportPhase === "success" ? (
              ...
              <>
                <div className="results-modal__header">
                  <div>
                    <h2 id="submission-report-title">
                      Report this {getRatingLabel(receivedRating)} rating?
                    </h2>
                    <p id="submission-report-description">
                      We&apos;ll review this rating and your answers for{" "}
                      {submission?.productName ?? "this app"}, and make things right. Thanks for
                      reporting!
                    </p>
                  </div>
                </div>
