    ...
    <AppShell eyebrowLabel={null}>
      <VerificationFlowShell title="Verify your email" cardClassName="verify-panel">
        ...
        <div className="inline-actions verify-actions">
          <button type="button" className="button button--primary" onClick={handleVerify} disabled={isSubmitting || !code.trim()}>
            Verify and continue
          </button>
          <button type="button" className="button button--secondary" onClick={() => void resend()} disabled={isSubmitting}>
            <RefreshCcw size={16} />
            Resend code
          </button>
        </div>
