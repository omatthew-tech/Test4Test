...
const defaultGroup3AppBaseUrl = "https://beta.test4test.io";

function getGroup3AppBaseUrl() {
  return (Deno.env.get("GROUP3_APP_BASE_URL")?.trim() || defaultGroup3AppBaseUrl).replace(
    /\/+$/,
    "",
  );
}
...
export async function loadDueReportShareReminders(admin: SupabaseClient, limit: number) {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, report_id, owner_user_id, recipient_user_id, recipient_name, recipient_email, reminders_sent, next_reminder_at",
    )
    .eq("status", "sent")
...
export async function processReportShareReminder(
  ...
  const senderName =
    normalizeName(sender?.display_name) || sender?.email?.trim() || "A Test4Test user";
  const reportName = report.report_name?.trim() || `Report ${report.report_number}`;
  const productName =
    (submissionData as { product_name?: string | null } | null)?.product_name?.trim() || "your app";
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "The report reminder could not be sent.";
