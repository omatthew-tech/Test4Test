create table if not exists public.feedback_rating_reports (
  ...
  unique (test_response_id, reporter_user_id)
);

alter table public.feedback_rating_reports enable row level security;
...
grant select, insert, update on public.feedback_rating_reports to authenticated;
